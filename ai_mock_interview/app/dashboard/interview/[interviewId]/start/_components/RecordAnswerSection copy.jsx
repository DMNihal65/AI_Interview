...
function RecordAnswerSection({ mockInterviewQuestions, activeQuestionIndex, interviewData }) {
    ...
    const handleSubmit = async () => {
        if (!mockInterviewQuestions || !interviewData) {
            setError('Interview data is missing. Please try reloading the page.');
            return;
        }
        ...
        try {
            const currentQuestion = mockInterviewQuestions.interview_questions[activeQuestionIndex];
            if (!currentQuestion) {
                throw new Error('Question not found. Please try reloading the page.');
            }
            ...
            if (!jsonResponse || typeof jsonResponse.rating !== 'number' || typeof jsonResponse.feedback !== 'string') {
            ...
            if (resp) {
                ...
                throw new Error('Failed to save the answer. Please try again.');
            }
        } catch (error) {
    ...
    if (!mockInterviewQuestions || !interviewData) {
        return <div className="text-red-500">Error: Interview data is missing. Please try reloading the page.</div>;
    }
